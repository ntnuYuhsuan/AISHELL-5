...
def get_parser():
    ...
    parser.add_argument(
        "--out-path", type=str, required=True, help="File path of ground truth results."
    )
...
def main():
    ...
    out_cpCER_file = args.out_path
    with open(out_cpCER_file, "w") as wf:
        ...
        for idx, sname in enumerate(hyp_sessions):
            ...
            wf.write("Computing: {}/{}, cpCER is: {:.4f}\n".format(
                    idx + 1, len(hyp_sessions), cost / entire
                ))
            cost_total += cost
            entire_total += entire
